...
(function (root) {
  ...
  var CATEGORIAS = ["Hemograma", "Función renal", "Electrolitos", "Perfil lipídico", "Metabólico",
    ...
    "Orina", "Gases arteriales", "Autoinmunidad", "Otros"];
  ...
  var RESTAURAR_ECO = [
    ...
    [/\bpatron\b/gi, "patrón"], [/\bparadojico\b/gi, "paradójico"], [/\bfuncion\b/gi, "función"], [/\bdisfuncion\b/gi, "disfunción"],
  ...
  var RESTAURAR = [
    [/\bpatron\b/gi, "patrón"], [/\bparadojico\b/gi, "paradójico"], [/\bhomogeneo\b/gi, "homogéneo"], [/\bcentromerico\b/gi, "centromérico"],
  ...
  function valorSeveridad(crudo, p) {
    ...
    return { valor: restaurarSev(frase), uni: "", fin: ms.index + ms[0].length, cualitativo: true };
  }
  ...
  function extraer(crudo, prep, p) {
    if (p.sev) {
      ...
      if (num && /\d/.test(String(num.valor))) {
        var sevAntes = valorSeveridad(crudo, p);
        var iNum = crudo.search(/-?\d/);
        if (sevAntes && iNum >= 0 && crudo.toLowerCase().indexOf(sevAntes.valor.split(" ")[0].toLowerCase()) < iNum) {
          return { v: sevAntes, sobre: crudo };
        }
  ...
  function redactar(datos, opts) {
    ...
    function pieza(r) {
      ...
      if (/^\d+:\d+$/.test(v)) return r.nom + " " + v + nota;                 // título 1:640
