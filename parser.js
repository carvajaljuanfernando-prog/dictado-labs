...
(function (root) {
  ...
  var RESTAURAR = [
    [/\bpatron\b/gi, "patrón"], [/\bhomogeneo\b/gi, "homogéneo"], [/\bcentromerico\b/gi, "centromérico"],
  ...
  function valorDe(prep, p) {
    ...
    // 5) resultado cualitativo de inmunología: positivo / negativo / débilmente positivo
    if (p.cuali) {
  ...
  function analizar(textoOriginal) {
    ...
    function repartirSobra(texto, fila) {
      ...
      if (fila && !/\d/.test(t) && t.length <= 40 && /^[a-zñ\s]+$/i.test(t)) { fila.nota = restaurar(t); pendiente = null; return; }
      guardarSobra(t);
    }
